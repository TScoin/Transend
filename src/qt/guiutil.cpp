...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Transend address (e.g. %1)").arg("AWSbBnzmNkjDVaYHX7vkL1MqD96pRYWtZo"));
#endif
...
{
    // return if URI is not valid or is no Transend: URI
...
{
    // Convert Transend:// to Transend:
    //
    //    Cannot handle this later, because Transend:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open Transend.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Transend.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Transend.lnk
...
{
    return GetAutostartDir() / "Transend.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a Transend.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Transend\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Transend app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add Transend app to startup item list
