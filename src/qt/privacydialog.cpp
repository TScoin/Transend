                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zTSC ought to be enough for anybody." - Bill Gates, 2017
    ui->zTSCpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzTSCSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzTSC->setEnabled(false);
        ui->pushButtonMintzTSC->setToolTip(tr("zTSC is currently disabled due to maintenance."));

        ui->pushButtonSpendzTSC->setEnabled(false);
        ui->pushButtonSpendzTSC->setToolTip(tr("zTSC is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zTSCpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzTSC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTSC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zTSC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zTSC in ") + 
...
void PrivacyDialog::on_pushButtonSpendzTSC_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTSC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zTSC
        sendzTSC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zTSC
    sendzTSC();
}

void PrivacyDialog::on_pushButtonZTSCControl_clicked()
{
    ZTSCControlDialog* zTSCControl = new ZTSCControlDialog(this);
    zTSCControl->setModel(walletModel);
    zTSCControl->exec();
}

void PrivacyDialog::setZTSCControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzTSCSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzTSC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Transend Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zTSCpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zTSC is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " TSC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zTSC</b>";
    ...
    // use mints from zTSC selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZTSCControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZTSCControlDialog::GetSelectedMints();
    }
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zTSC transaction
    ...
    // Clear zTSC selector in case it was used
    ZTSCControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zTSC Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " TSC, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zTSC Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTSC </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zTSC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zTSC "));
    ui->labelzTSCAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzTSCSyncStatus->setVisible(fShow);
}
