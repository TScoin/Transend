...
class CTxIn;
class CHodgepodgePool;
class CHodgepodgeSigner;
class CMasterNodeVote;
class CBitcoinAddress;
class CHodgepodgeQueue;
class CHodgepodgeBroadcastTx;
...
#define Hodgepodge_QUEUE_TIMEOUT 30
#define Hodgepodge_SIGNING_TIMEOUT 15

// used for anonymous relaying of inputs/outputs/sigs
#define Hodgepodge_RELAY_IN 1
#define Hodgepodge_RELAY_OUT 2
#define Hodgepodge_RELAY_SIG 3

static const CAmount Hodgepodge_COLLATERAL = (10 * COIN);
static const CAmount Hodgepodge_POOL_MAX = (99999.99 * COIN);

extern CHodgepodgePool HodgepodgePool;
extern CHodgepodgeSigner HodgepodgeSigner;
extern std::vector<CHodgepodgeQueue> vecHodgepodgeQueue;
extern std::string strMasterNodePrivKey;
extern map<uint256, CHodgepodgeBroadcastTx> mapHodgepodgeBroadcastTxes;
extern CActiveMasternode activeMasternode;

/** Holds an Hodgepodge input
 */
...
// A clients transaction in the Hodgepodge pool
class CHodgepodgeEntry
{
public:
    bool isSet;
    ...
    /// Add entries to use for Hodgepodge
    ...
    bool IsExpired()
    {
        return (GetTime() - addedTime) > Hodgepodge_QUEUE_TIMEOUT; // 120 seconds
    }
};


/**
 * A currently inprogress Hodgepodge merge and denomination information
 */
class CHodgepodgeQueue
{
public:
    ...
    /** Sign this Hodgepodge transaction
    ...
    /// Is this Hodgepodge expired?
    bool IsExpired()
    {
        return (GetTime() - time) > Hodgepodge_QUEUE_TIMEOUT; // 120 seconds
    }
...
/** Helper class to store Hodgepodge transaction (tx) information.
 */
class CHodgepodgeBroadcastTx
{
public:
...
/** Used to keep track of current status of Hodgepodge pool
 */
class CHodgepodgePool
{
private:
    mutable CCriticalSection cs_Hodgepodge;

    std::vector<CHodgepodgeEntry> entries; // Masternode/clients entries
...
public:
    ...
    CHodgepodgePool()
    {
        /* Hodgepodge uses collateral addresses to trust parties entering the pool
    ...
    /** Process a Hodgepodge message using the Hodgepodge protocol
     * \param pfrom
     * \param strCommand lower case command string; valid values are:
     *        Command  | Description
     *        -------- | -----------------
     *        dsa      | Hodgepodge Acceptable
     *        dsc      | Hodgepodge Complete
     *        dsf      | Hodgepodge Final tx
     *        dsi      | Hodgepodge vIn
     *        dsq      | Hodgepodge Queue
     *        dss      | Hodgepodge Signal Final Tx
     *        dssu     | Hodgepodge status update
     *        dssub    | Hodgepodge Subscribe To
     * \param vRecv
     */
    void ProcessMessageHodgepodge(CNode* pfrom, std::string& strCommand, CDataStream& vRecv);

    void InitCollateralAddress()
    {
        SetCollateralAddress(Params().HodgepodgePoolDummyAddress());
    }
    ...
    void UpdateState(unsigned int newState)
    {
        if (fMasterNode && (newState == POOL_STATUS_ERROR || newState == POOL_STATUS_SUCCESS)) {
            // LogPrint("Hodgepodge", "CHodgepodgePool::UpdateState() - Can't set state to ERROR or SUCCESS as a Masternode. \n");
            return;
        }

        // LogPrintf("CHodgepodgePool::UpdateState() == %d | %d \n", state, newState);
        if (state != newState) {
    ...
    /// Passively run Hodgepodge in the background according to the configuration in settings (only for QT)
    bool DoAutomaticDenominating(bool fDryRun = false);
    bool PrepareHodgepodgeDenominate();

    /// Check for process in Hodgepodge
    void Check();
    ...
    void SendHodgepodgeDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, CAmount amount);
    /// Get Masternode updates about the progress of Hodgepodge
    ...
    /// Get the denominations for a specific amount of Transend.
