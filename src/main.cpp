...
#if defined(NDEBUG)
#error "Transend cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zTSC spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zTSC spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zTSC spend with serial %s from tx %s is not in valid range\n",
        ...
        if (mapHodgepodgeBroadcastTxes.count(hash)) {
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight < 480 && nHeight > 0) {
        nSubsidy = 5 * COIN;
    } else if (nHeight < 960 && nHeight >= 480) {
        nSubsidy = 10 * COIN;
    } else if (nHeight <= 1440 && nHeight >= 960) {
        nSubsidy = 15 * COIN;
    } else if (nHeight <= 2000 && nHeight >= 1440) {
        nSubsidy = 20 * COIN;
    } else if (nHeight <= 5000 && nHeight >= 2000) {
        nSubsidy = 25 * COIN;
    } else if (nHeight <= 388799 && nHeight >= 5000) {
        nSubsidy = 0.90 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Transend
...
{
    RenameThread("Transend-scriptch");
...
void RecalculateZTSCSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zTSC supply
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zTSC with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zTSCSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert TSC to zTSC
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zTSC spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zTSC serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zTSC mints and %d zTSC spends\n", __func__, nMints, nSpends);
...
bool static AlreadyHave(const CInv& inv)
{
    ...
    case MSG_DSTX:
        return mapHodgepodgeBroadcastTxes.count(inv.hash);
    case MSG_BLOCK:
        ...
        const CInv& inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK) {
                ...
                if (!pushed && inv.type == MSG_DSTX) {
                    if (mapHodgepodgeBroadcastTxes.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mapHodgepodgeBroadcastTxes[inv.hash].tx << mapHodgepodgeBroadcastTxes[inv.hash].vin << mapHodgepodgeBroadcastTxes[inv.hash].vchSig << mapHodgepodgeBroadcastTxes[inv.hash].sigTime;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Transend: We use certain sporks during IBD, so check to see if they are
        ...
        if (strCommand == "tx") {
            ...
            if (pmn != NULL) {
                ...
                if (!mapHodgepodgeBroadcastTxes.count(tx.GetHash())) {
                    ...
                    mapHodgepodgeBroadcastTxes.insert(make_pair(tx.GetHash(), dstx));
                }
            }
        }
    ...
    // getaddr message mitigates the attack.
        ...
        HodgepodgePool.ProcessMessageHodgepodge(pfrom, strCommand, vRecv);
